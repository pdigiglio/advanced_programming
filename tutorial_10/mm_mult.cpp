...
estimate_mm_mult_performance(
  /* machine properties */
	...
	for ( unsigned int vector_size = 1; vector_size <= 8; vector_size *= 2 ) {
		 ...
		 */
		double expected_performance = computational_intensity * double_memory_bandwidth;
...
//		for ( unsigned int i = 0; i < k; ++ i ) {
			...
			/** First assign initial values to the \f$C\f$ matrix. */
			for ( unsigned int row = 0; row < rows; ++ row ) {
				C[ row * columns + col ] = A[ row * k ] * B[ col ];
			}
//		}
 ...
 * @brief
 *
 * @attention I am assuming that `blocksize` divides the number of rows and columns of
 * each matrix!
 */
	void
mm_mult_rck_block( size_t rows, size_t columns, size_t k, double *A, double *B, double *C, size_t blocksize ) {


	/**
	 * Initialize the resulting matrix.
	 */
	for ( unsigned int bb = 0; bb < columns / blocksize; ++ bb ) {

		for ( unsigned int b = 0; b < rows / blocksize; ++ b ) {

			for ( unsigned int r = b * blocksize; r < ( b + 1 ) * blocksize; ++ r ) {

				for ( unsigned int c = bb * blocksize; c < (bb + 1 ) * blocksize; ++ c ) {

					// initialize the value with the first element of the sum
					C[ r * columns + c ] = A[ r * k ] * B[ c ];
					for ( unsigned int i = 0; i < blocksize; ++ i ) {
						C[ r * columns + c ] += A[ r * k + i ] * B[ i * columns + c ];
					}
				}
			}
		}
	...
	for ( unsigned int bbb = 1; bbb < k / blocksize; ++ bbb ) {
			...
			for ( unsigned int b = 0; b < rows / blocksize; ++ b ) {
				...
				for ( unsigned int r = b * blocksize; r < ( b + 1 ) * blocksize; ++ r ) {
					...
					for ( unsigned int c = bb * blocksize; c < (bb + 1 ) * blocksize; ++ c ) {
						...
						for ( unsigned int i = bbb * blocksize; i < ( bbb + 1 ) * blocksize; ++ i ) {
							C[ r * columns + c ] += A[ r * k + i ] * B[ i * columns + c ];
						}
					}
				}
...
testFunction( mm_mult_ptr mm_mult, size_t rows, size_t columns, size_t k, size_t samples, double *A, double *B, double *C) {
	...
	for ( unsigned int sample = 0; sample < samples; ++ sample ) {
		...
		/** Take again the current time */
		end = std::chrono::system_clock::now();

		std::chrono::duration<double> elapsed_seconds = end - start;
		
		/**
		 * The number of operation is known and also the elapsed time so one can evaluate
		 * the actual value of _GFLOPs/second_.
		 */
...
/**
 * @brief Test function for blocking matrix product.
 *
 * The function is actually completely equal to `testFunction()` but a different function
 * pointer argument is needed so it has been implemented again.
 *
 * @param mm_mult function pointer to one of the matrix-multiplication functions previously
 * implemented
 * @param rows rows of the first matrix
 ...
 * @param k columns of the first matrix = rows of the second matrix
 * @param samples number of times `mm_mult` is called
 * @param blocksize the size of the matrix blocks
 * @param A first matrix pointer
 * @param B second matrix pointer
 * @param C resulting matrix pointer
 */
...
memory_bandwidth_check( size_t rows, size_t columns, double *src, double *dest ) {
	...
	/**
	 * Teinterpret cast to treat the `double` function parameters as strings and use 
	 * `memcpy()` on them to see how long it takes.
	 */
	...
	/// There are two cases:
	/// 	1. memory size is too big for cache, than there will be almost no difference
	/// 	2. memory size fits into cache, than all further calls will be a lot faster
